...
function showSingleMainMenu(e, p) {
    ...
    form.show(p).then(data => {
        ...
        if (dv == 0) {
            e.setProperty('viberater:active', 1)
            e.setDynamicProperty('time', Date.now() + (86400000 * (10 + Math.min(20, Math.floor((Date.now() - e.getDynamicProperty('initialTime')) / 86400000)))))
    ...
    form.show(p).then(data => {
        ...
        if (dv == 1) {
            getLCCs(gCL0, gCL1, gCLs, p).then(lccs => {
                ...
                lccs.forEach(lcc => {
                    if (lcc?.getDynamicProperty('owner') == p.id) {
    ...
    form.show(p).then(data => {
        ...
        if (dv == 0) {
            getLCCs(gCL0, gCL1, gCLs, p).then(lccs => {
                const cCC = lccs.filter(lcc => lcc?.getDynamicProperty('owner') === p.id).length
                lccs.forEach(lcc => {
                    if (lcc?.getDynamicProperty('owner') === p.id) lcc.setDynamicProperty('allowListEnabled', false)
                })

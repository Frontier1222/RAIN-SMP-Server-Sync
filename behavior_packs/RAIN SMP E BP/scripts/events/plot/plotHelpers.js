...
export function getPlayerOwnedClaims(player) {
    ...
    return getAllGlobalClaims().filter(
        (p) =>
            ...
            !p.isSubclaim &&
            !p.factionClaim &&
            (p.ownerId === player.id || String(p.ownerName || '').toLowerCase() === playerName)
    );
}
...
/** Give the claim wand and remember which slot it lives in. */
export function equipPlotClaimWand(player, slot = player?.selectedSlotIndex) {
    if (!player) return false;

    const inv = player.getComponent('minecraft:inventory')?.container;
    if (!inv || typeof slot !== 'number') return false;
    ...
    inv.setItem(slot, new ItemStack('minecraft:wooden_axe', 1));
    player.setDynamicProperty('plot_wand_slot', slot);
    player.setDynamicProperty('plot_prev_slot', slot);
    return true;
}

/** Re-place the wand if creative purge or other systems removed it mid-claim. */
export function ensurePlotClaimWand(player) {
    if (!player?.hasTag('plot_making')) return false;

    const inv = player.getComponent('minecraft:inventory')?.container;
    if (!inv) return false;

    for (let i = 0; i < inv.size; i++) {
        if (inv.getItem(i)?.typeId === 'minecraft:wooden_axe') return true;
    }

    let slot = player.getDynamicProperty('plot_wand_slot');
    if (typeof slot !== 'number') slot = player.selectedSlotIndex;
...
export function clearPlotClaimWandState(player) {
    if (!player) return;
    player.setDynamicProperty('plot_wand_slot', undefined);
    player.setDynamicProperty('plot_prev_slot', undefined);
}

...
import { getAllGlobalClaims, addToGlobalClaims, rebuildGlobalClaimChunkCache, ensurePlotClaimWand, clearPlotClaimWandState } from './plotHelpers.js';
...
export default {
    ...
    run: async (ev) => {
        ...
        ensurePlotClaimWand(player);

        const inv = player.getComponent('minecraft:inventory')?.container;
        const held = inv?.getItem(player.selectedSlotIndex) ?? itemStack;
        if (!held || held.typeId !== 'minecraft:wooden_axe') return;

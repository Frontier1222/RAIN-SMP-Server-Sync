...
import { isTester, isRestrictedCreativeRole } from "./creativeRoleGuard.js";
...
function applyArmorStandRestore(snapshot, player) {
    ...
    try {
        ...
        let entity = findArmorStand(snapshot);
        if (!entity?.isValid) {
            entity = dim.spawnEntity("minecraft:armor_stand", loc);
        }
...
export function enforceProtectedArmorStand(player, entity, denyFn) {
    ...
    if (snap) {
        ...
        scheduleRestoreJob(enforceKey, () => {
            let stand = findArmorStand(snap);
            if (stand?.isValid) return true;

...
function showSingleMainMenu(e, p) {
    ...
    if (time != -1) {
        ...
        form.body(`Infinite Protection Time.`)
    }
    ...
    form.show(p).then(data => {
        ...
        if (dv == 3) {
            ...
            gCL.forEach((line, index) => {
                addLine(line, undefined, { r: 0, g: 1, b: 0, a: 1 }, e.dimension, e.id.toString() + "_" + index.toString(), [p], 396)
            })
        } else {
            const gCL = getChunkLines(e)
            gCL.forEach((line, index) => {
        ...
        if (wsg.state == 'selectSecondChunk') {
            ...
            gCLs.forEach((gCL, index1) => {
                gCL.forEach((line, index2) => {
                    addLine(line, undefined, { r: 1, g: 1, b: 1, a: 1 }, p.dimension, p.id.toString() + "_" + "gCLs" + "_" + index1.toString() + "_" + index2.toString(), [p], 396)
                })
            })
            wandStates.set(p.id, {
                    ...
                    lccs.forEach((lcc, index) => {
                        if (!lcc) {
                            ...
                            getChunkLines(e).forEach((fc, index) => {
                                addLine(fc, 5, { r: 0, g: 1, b: 0, a: 1 }, p.dimension, undefined, [p], 396)
                            })
                        }
                    })
            ...
            getLCCs(gCL0, gCL1, gCLs, p).then(lccs => {
                ...
                lccs.forEach(lcc => {
                    if (lcc?.getDynamicProperty('owner') == p.id || !pncm) {
                        if (lcc.setDynamicProperty('time') == -1) return
        ...
        if (dv == 0) {
            ...
            if (placedCount > 16 && pncm) {
                ...
                if (pncm) {
                    ...
                    p.sendMessage(`§aYou've claimed a chunk for the server!`)
                }
            }
        }
    })

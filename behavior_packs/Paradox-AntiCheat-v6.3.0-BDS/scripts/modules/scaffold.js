...
const playerBlockPlacements = new Map();
// Variables to store the subscription references
let blockPlacementCallback;
 ...
 */
function detectScaffolding(playerId) {
    ...
    let xMatch = 1, yMatch = 1, zMatch = 1;
        ...
        if (suspiciousBlocks.length > 0) {
            system.run(() => {
                ...
                suspiciousBlocks.forEach((pos) => {
                    ...
                    if (suspiciousBlock)
                        suspiciousBlock.setType("minecraft:air");
                });
            });
        }
    };

...
import { isClaimPermAllowed, plotAllowsTesterBuildBypass, plotAllowsTesterShulkerBypass, hasTesterClaimBuildBypass, canTesterBypassRestrictedZone, plotHidesEnterToastForRole } from "./utils/claimPermissions.js";
...
const SIDEBAR_LOOP_TICKS = 20;
const NAMETAG_EVERY_LOOPS = 4;
...
function tryPerformCaptureCube(player, target, event) {
    ...
    if (plot && !hasPlotPermission(plot, player, "protectEntityKill")) {
        event.cancel = true;
        system.run(() => {
            toastDeny(
                player,
...
if (world.beforeEvents && world.beforeEvents.chatSend) {
    world.beforeEvents.chatSend.subscribe((ev) => {
        ...
        if (
            ...
            system.run(() => {
                if (player.hasTag("creative_builder") && !player.hasTag("staff")) {
                    notify(
                        player,
                        "tpa_builder_block",
...
function tickPlotEnterSafezoneStaggered(players) {
    ...
    const batch = Math.min(PLOT_ENTER_BATCH, players.length);
...
function tickCombatElytra(players, combatRemainingById) {
    for (const player of players) {
        if ((combatRemainingById.get(player.id) ?? 0) <= 0) continue;
...
// --- THE ENFORCERS ---
if (world.beforeEvents && world.beforeEvents.entityDamage) {
    world.beforeEvents.entityDamage.subscribe((event) => {
        try {
            ...
            if (victim?.typeId === "minecraft:player" && attacker?.typeId === "minecraft:player") {
                ...
                const plot = getPlotAtLocation(victim.location, victim.dimension.id);
                if (plot && !hasPlotPermission(plot, attacker, "protectPvp")) {
                    event.cancel = true;
                    system.run(() => {
                        notify(attacker, "claim_no_pvp", `§r§c§l[CLAIM]§r §cPVP is disabled inside ${plot.name || "this claim"}!`, "", "note.bass");
                    });
                }
            }
        } catch (e) { }
...
function vacuumCreativeBuilderDropEntity(ent, attempt = 0) {
    ...
    try {
        ...
        for (const player of closePlayers) {
            ...
            const isBuilder = player.hasTag("creative_builder") && !(player.hasTag("staff") && !isTester(player));
            if (isBuilder) {
                stashItemStackToBuilderVault(player, stack);
            }

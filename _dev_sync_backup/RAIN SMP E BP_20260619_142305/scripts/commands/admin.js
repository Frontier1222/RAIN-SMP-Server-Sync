...
import { hasPermission, BUILTIN_RANKS, ROLE_PERMISSION_DEFS, UTILITY_ROLE_TAGS, PROTECTED_ROLE_IDS, invalidateCustomRolesCache, formatPlayerRoleSummary, getCustomRoles, getRankMeta } from '../systems/ranks.js';
...
async function applyHubGamemode(player, modeName) {
  ...
  const fromMode = getPlayerGamemodeKey(player);
  if (fromMode === modeName) return true;

  const finish = () => {
    if (getPlayerGamemodeKey(player) !== modeName) return false;

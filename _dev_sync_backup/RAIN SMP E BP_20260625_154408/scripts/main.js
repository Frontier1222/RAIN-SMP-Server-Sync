...
import { formatNameTag, getRankMeta, formatChatLine } from "./systems/ranks.js";
import { rebuildGlobalClaimChunkCache, getGlobalPlotAtLocation, syncOwnedPlotsFromGlobal } from "./events/plot/plotHelpers.js";
import { enforcePlotEnterDenyForPlayer } from "./events/plot/plotProtection.js";
import { isClaimPermAllowed, plotAllowsTesterBuildBypass, plotAllowsTesterShulkerBypass, hasTesterClaimBuildBypass, canTesterBypassRestrictedZone, plotHidesEnterToastForRole, isPvpAllowedInClaim, isPvpDisabledInClaim } from "./utils/claimPermissions.js";
...
import { isLivePlayer } from "./utils/livePlayer.js";
import {
    preloadChunkThen,
    findSafeSurfaceY,
    isPlayerOnline,
...
const ACTIONBAR_STAT_REFRESH_LOOPS = 4;
...
function isBuilderOrTester(player) {
    ...
    const isStaff = player.hasTag("staff") && !_isTester;
    const isBuilder = player.hasTag("creative_builder") && !isStaff;
    return isBuilder || _isTester;
}
...
function isRestrictedBuilder(player) {
    return player.hasTag("creative_builder") && !player.hasTag("staff") && isPlayerInCreative(player);
}
...
function isBlockedCaptureMob(typeId) {
    const id = String(typeId || "").toLowerCase();
...
function isEntityTamedByOtherPlayer(entity, player) {
    if (!entity || !player) return false;

    try {
        const tameable = entity.getComponent("minecraft:tameable");
        const owner = tameable?.tamedToPlayer;
        if (owner?.id && owner.id !== player.id) return true;
    } catch (e) {}

    return false;
}

function denyCaptureOwnedMob(player) {
    system.run(() => {
        toastDeny(player, "§cYou cannot capture another player's tamed mob!", "capture_owned");
    });
}

function canCaptureMobInClaim(plot, player) {
    ...
    return isClaimPermAllowed(plot, player, "protectEntityKill");
}
...
function tryBlockCaptureCubeUse(player, target, event) {
    ...
    if (isBlockedCaptureMob(target.typeId)) {
        event.cancel = true;
        denyBlockedCapture(player, target);
        return true;
    }
    if (isEntityTamedByOtherPlayer(target, player)) {
        event.cancel = true;
        denyCaptureOwnedMob(player);
        return true;
    }
    return false;
}
...
function tryPerformCaptureCube(player, target, event) {
    ...
    if (isBlockedCaptureMob(target.typeId)) return false;
    if (isEntityTamedByOtherPlayer(target, player)) {
        event.cancel = true;
        denyCaptureOwnedMob(player);
        return true;
    ...
    const plot = getPlotAtLocation(target.location, target.dimension.id);
    if (plot && !canCaptureMobInClaim(plot, player)) {
        event.cancel = true;
        system.run(() => {
            toastDeny(
                player,
                `§cYou cannot capture mobs in ${plot.name || "this claim"}!`,
...
function tryReleaseCaptureCube(player, itemStack, cancelEvent) {
    ...
    if (isBlockedCaptureMob(entityType)) {
...
function isBannedOreScanner(typeId) {
    if (!typeId) return false;
    const id = String(typeId).toLowerCase();
    return id.includes("ulkd_ess:detector") || id.includes("ore_scanner") || id.includes("orescanner");
}

function isRestrictedBundleItem(typeId) {
    if (!typeId) return false;
    const id = String(typeId).toLowerCase();
    return id === "minecraft:bundle" || id.endsWith("_bundle");
}
...
function blockWorldBuilderStaffOnlyUse(player, itemStack, cancelEvent) {
    ...
    system.run(() => {
        notify(
            player,
            "wb_staff_only",
            "§r§c§l[RESTRICTED]§r §cWorld Builder tools require the world_builder tag.",
            "",
            "note.bass"
        );
    });
...
function tickActionbarHud(players) {
    ...
    for (const player of batch) {
        if (player.hasTag("hide_stats")) continue;
...
function getParadoxCommandPrefix() {
    try {
...
/** Paradox AntiCheat registers __prefix on world load and owns : commands + chat relay. */
function isParadoxChatActive() {
    try {
        return world.getDynamicProperty("__prefix") !== undefined;
    } catch (e) {
        return false;
    }
}
...
if (world.beforeEvents && world.beforeEvents.chatSend) {
    world.beforeEvents.chatSend.subscribe((ev) => {
        ...
        if (gmMatch && isBuilderOrTester(player)) {
            const fromMode = getSafeGamemode(player);
            const targetMode = parseGamemodeCommandArg(gmMatch[1]);
            if (fromMode !== "unknown" && targetMode && fromMode !== targetMode) {
                system.runTimeout(() => {
                    const toMode = getSafeGamemode(player);
                    if (toMode !== "unknown" && toMode === targetMode && fromMode !== toMode) {
        ...
        if (msg === "!fixkit" || msg === "-fixkit") {
            ...
            system.run(() => {
                ...
                if (invComp && invComp.container) {
                    // ✨ OPTIMIZATION: Check lockMode before saving to reduce ticks
        ...
        if (
            ...
            system.run(() => {
                if (player.hasTag("creative_builder") && !player.hasTag("staff") && isPlayerInCreative(player)) {
                    notify(
                        player,
                        "tpa_builder_block",
        ...
        // Paradox : commands (and chat relay when the pack is active)
        if (rawMsg.startsWith(getParadoxCommandPrefix()) || isParadoxChatActive()) {
            return;
        }
        ...
        system.run(() => {
            try {
                const line = formatChatLine(player, ev.message);
                if (line) world.sendMessage(line);
            } catch (e) {
                world.sendMessage(`§7${player.name}: ${ev.message}`);
            }
        });
    ...
    world.afterEvents.playerSpawn.subscribe((event) => {
        ...
        if (event.initialSpawn) {
            ...
            system.runTimeout(() => {
                ...
                if (!canUseWorldBuilderTools(player)) {
                    try { player.runCommand(`clear @s ${WORLD_BUILDER_HANDBOOK}`); } catch (e) { }
                }
            }, 20);
            system.runTimeout(() => {
                if (!isLivePlayer(player)) return;
        ...
        if (isArenaMinigamePlayer(player)) {
            ...
            system.runTimeout(() => {
                ...
                player.removeTag("arena_player_1"); player.removeTag("arena_player_2");
                ensureRainGuiItem(player);
            ...
            if (winner) {
                winner.removeTag("in_arena");
                winner.removeTag("arena_pvp");
                system.runTimeout(() => {
                    restoreInventory(winner);
                    winner.teleport(FRONT_OF_ARENA, { dimension: world.getDimension("overworld") });
                    winner.removeTag("arena_player_1"); winner.removeTag("arena_player_2");
            ...
            system.runTimeout(() => {
                restoreInventory(player);
                player.removeTag("spleef_p1"); player.removeTag("spleef_p2"); player.removeTag("spleef_p3"); player.removeTag("spleef_p4");
                ensureRainGuiItem(player);
                notify(player, "spleef_death", "§r§c§l[SPLEEF]§r §cYou fell! Your items have been safely restored.");
            }, 10);
        }
        else if (handleSoccerPlayerRespawn(player)) {
            ensureRainGuiItem(player);
        }
    });
}
...
function tickPlotEnterSafezone(player) {
    try {
        ...
        if (plot && isPvpDisabledInClaim(plot)) {
            if (!player.hasTag("in_safezone")) player.addTag("in_safezone");
        } else {
            if (player.hasTag("in_safezone")) player.removeTag("in_safezone");
        }

        const visiblePlot = plot?.isSubclaim ? null : plot;
        const visibleLastPlot = lastPlot?.isSubclaim ? null : lastPlot;
    ...
    if (arenaPlayers.length === 1) {
        ...
        if (!arenaWaitTime.has(p.id)) {
            ...
            if (elapsed >= 30000) {
                ...
                p.removeTag("in_arena");
                p.removeTag("arena_pvp");
                p.removeTag("arena_player_1");
                p.removeTag("arena_player_2");
    ...
    if (spleefPlayers.length === 1) {
        ...
        if (!spleefWaitTime.has(p.id)) {
            ...
            if (elapsed >= 30000) {
                ...
                p.removeTag("in_spleef");
                p.removeTag("spleef_p1");
                p.removeTag("spleef_p2");
                p.removeTag("spleef_p3");
                p.removeTag("spleef_p4");
...
function tickWorldBorderForPlayer(player, now) {
    if (player.hasTag("staff") && !isTester(player)) return;
...
function runPlayerItemMaintenance(player, { stripWorldBuilder = false } = {}) {
    ...
    const stripWb = stripWorldBuilder && !canUseWorldBuilderTools(player);
    ...
    if (equippable) {
        for (const slot of ITEM_MAINT_EQUIP_SLOTS) {
            ...
            if (stripBanned && !player.hasTag("staff") && (isBannedOreScanner(typeId) || isRestrictedBundleItem(typeId))) {
                equippable.setEquipment(slot, undefined);
                bannedRemoved = true;
                continue;
            }
...
function schedulePlayerItemMaintenance(player) {
    ...
    system.run(() => {
        ...
        if (isPlayerOnline(player)) runPlayerItemMaintenance(player, { stripWorldBuilder: false });
    });
}
...
if (world.beforeEvents && world.beforeEvents.playerBreakBlock) {
    world.beforeEvents.playerBreakBlock.subscribe((event) => {
        try {
            ...
            if (isBannedBerry(blockId) && !player.hasTag("staff")) {
                event.cancel = true;
                system.run(() => {
    ...
    world.beforeEvents.itemUse.subscribe((event) => {
        try {
            ...
            if (isCaptureCubeItem(event.itemStack?.typeId)) {
                ...
                if (target && isBlockedCaptureMob(target.typeId)) {
                    event.cancel = true;
                    denyBlockedCapture(player, target);
                    return;
                }
                if (target && tryPerformCaptureCube(player, target, event)) {
                    return;
                }
            }
...
if (world.beforeEvents && world.beforeEvents.playerInteractWithEntity) {
    world.beforeEvents.playerInteractWithEntity.subscribe((event) => {
        try {
            ...
            if (tryPerformCaptureCube(player, target, event)) return;

            const heldForGive = event.itemStack;
            if (heldForGive && target?.typeId === "minecraft:player" && !canUseWorldBuilderTools(player)) {
                if (isWorldBuilderStaffOnlyItem(heldForGive.typeId)) {
                    event.cancel = true;
                    system.run(() => {
                        notify(
                            player,
                            "wb_give_block",
                            "§r§c§l[RESTRICTED]§r",
                            "§cWorld Builder books and tools cannot be given to other players.",
                            "",
                            "note.bass"
                        );
                    });
...
if (world.beforeEvents && world.beforeEvents.playerInteractWithBlock) {
    world.beforeEvents.playerInteractWithBlock.subscribe((event) => {
        try {
            ...
            if (isBannedBerry(blockId) && (!player.hasTag("staff") || isTester(player))) {
                event.cancel = true;
                system.run(() => {
                    notify(player, "banned_berry", "§r§c§l[SERVER]§r §cThese ore berries are completely banned!", "", "note.bass");
                });
...
function vacuumCreativeBuilderDropEntity(ent, attempt = 0) {
    ...
    try {
        const dropperId = ent.getDynamicProperty("nf.dropper_id");
        let dropper = null;

        if (dropperId !== undefined && dropperId !== null) {
            dropper = getRealmPlayerById(Number(dropperId));
        }

        if (!dropper?.isValid || !shouldBlockCreativeRoleDrop(dropper)) return;
        if (isInTotemLimiterDropGrace(dropper)) return;

        const isBuilder = dropper.hasTag("creative_builder") && !(dropper.hasTag("staff") && !isTester(dropper));
        if (isBuilder) {
            stashItemStackToBuilderVault(dropper, stack);
        }
        ...
        system.run(() => {
            denyRestrictedCreative(
                ...
                "Dropped items are stored in your builder vault."
            );
        });
    } catch (e) { }
}

if (world.afterEvents?.playerDropItem) {
    world.afterEvents.playerDropItem.subscribe((event) => {
        const player = event.source;
        const entity = event.itemEntity;
        if (!player || !entity?.isValid) return;
        try {
            entity.setDynamicProperty("nf.dropper_id", player.id);
        } catch (e) {}
        if (shouldBlockCreativeRoleDrop(player)) {
...
function restoreInventory(player) {
    ...
    // ✨ FIX: Unlock all minigame kit items first to prevent 'Item is locked' crashes when clearing!
    for (let i = 0; i < container.size; i++) {
        const item = container.getItem(i);
        if (item) {
    ...
    // ✨ FIX: Unlock equipment (kit armor) too!
    const slots = [EquipmentSlot.Head, EquipmentSlot.Chest, EquipmentSlot.Legs, EquipmentSlot.Feet, EquipmentSlot.Offhand];
    for (const slot of slots) {
...
function executeVaultSwap(player, isStoring, vz, dim) {
    try {
        ...
        if (!b1 || !b2) {
            ...
            try { player.runCommandAsync("effect @s blindness 2 255 true"); } catch (e) { }
            player.teleport({ x: VAULT_X, y: VAULT_Y + 2, z: vz }, { dimension: dim });

            system.runTimeout(() => {
                try {
                    b1 = dim.getBlock({ x: VAULT_X, y: VAULT_Y, z: vz });
                    b2 = dim.getBlock({ x: VAULT_X, y: VAULT_Y, z: vz + 1 });
                    if (b1 && b1.typeId !== "minecraft:barrel") b1.setType("minecraft:barrel");
                    if (b2 && b2.typeId !== "minecraft:barrel") b2.setType("minecraft:barrel");

                    const vComp1 = b1?.getComponent("inventory") || b1?.getComponent("minecraft:inventory");
                    const vComp2 = b2?.getComponent("inventory") || b2?.getComponent("minecraft:inventory");

                    if (vComp1 && vComp2) performActualSwap(player, isStoring, vComp1, vComp2);
        ...
        if (!vComp1 || !vComp2) {
            notify(player, "vault_building", "§r§e§l[SYSTEM]§r §eVault is building its storage... Switch modes once more to confirm!");
            return;
        }

        performActualSwap(player, isStoring, vComp1, vComp2);
    } catch (e) {
        notify(player, "vault_crash", `§r§c§l[ERROR]§r §cVault Crash: ${e}`, "", "note.bass");
    }
}
...
function applyBuilderTesterGamemodeChange(player, fromMode, toMode) {
    ...
    if (toMode === "creative" && fromMode !== "creative") {
        if (isBuilder && builderHasStashableItems(player)) {
            handleVault(player, true);
        }
...
function tickGamemodeTracking(players) {
    for (const player of players) {
        ...
        syncCreativeRoleTag(player);
        const currentMode = getSafeGamemode(player);
        const lastMode = getRememberedGamemode(player);

        if (currentMode !== "unknown" && lastMode !== currentMode) {
            applyBuilderTesterGamemodeChange(player, lastMode ?? "survival", currentMode);
        } else if (currentMode !== "unknown") {
            rememberPlayerGamemode(player, currentMode);
        }
    }
}
...
world.afterEvents.playerInteractWithBlock.subscribe((event) => {
    ...
    if (block.location.x === BUTTON_PLAYER_1.x && block.location.y === BUTTON_PLAYER_1.y && block.location.z === BUTTON_PLAYER_1.z) {
        if (player.hasTag("in_arena") || player.hasTag("in_spleef") || player.hasTag("in_soccer")) return;
    ...
    for (const btn of spleefButtons) {
        if (block.location.x === btn.loc.x && block.location.y === btn.loc.y && block.location.z === btn.loc.z) {
            if (player.hasTag("in_spleef") || player.hasTag("in_arena") || player.hasTag("in_soccer")) return;
    ...
    world.beforeEvents.itemUse.subscribe((event) => {
        ...
        if (event.itemStack && event.itemStack.typeId === "minecraft:ender_pearl") {
            ...
            if (now - lastUsed < ENDER_PEARL_COOLDOWN_MS) {
                event.cancel = true; // Stop the pearl from throwing
                const timeLeft = ((ENDER_PEARL_COOLDOWN_MS - (now - lastUsed)) / 1000).toFixed(1);
                
                system.run(() => {
                    // Uses your existing custom notify function!
                    notify(
                        player, 
                        "pearl_cooldown", 
                        "§c§l[COOLDOWN]", 
                        ...
                        "note.bass", 
                        1000 // 1-second toast anti-spam delay
                    );
                });
            } else {
                // Update cooldown timer if they are allowed to throw it
                enderPearlCooldowns.set(player.id, now);
            }
        }
    });
}

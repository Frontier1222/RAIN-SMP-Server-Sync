...
/** Arena/spleef use scripted inventory restore — gravestones already skip those tags. */
function shouldSkipRainGuiDeathHandling(player) {
    if (!player) return true;
    try {
        return player.hasTag("in_arena") || player.hasTag("in_spleef") || player.hasTag("in_soccer");
    } catch (e) {
        return false;
    }
}
...
function handleRainGuiPlayerDeath(player) {
    ...
    if (shouldSkipRainGuiDeathHandling(player)) return;

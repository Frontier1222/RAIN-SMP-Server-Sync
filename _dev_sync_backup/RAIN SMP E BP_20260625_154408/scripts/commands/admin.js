...
import { hasPermission, BUILTIN_RANKS, ROLE_PERMISSION_DEFS, UTILITY_ROLE_TAGS, PROTECTED_ROLE_IDS, invalidateCustomRolesCache, formatPlayerRoleSummary, getCustomRoles, getRankMeta, syncRainGuiBlockState, RAIN_GUI_BLOCK_KEY } from '../systems/ranks.js';
...
import { stripRainGuiForGravestone } from '../utils/rainGui.js';
...
function findBanEntry(player) {
  const name = safePlayerName(player).toLowerCase();
  const id = safePlayerId(player);
...
function setPrimaryRankTag(player, tag) {
  ...
  const role = getCustomRoles().find((r) => r.tag === tag || `rank:${r.id}` === tag);
  try {
    player.setDynamicProperty(RAIN_GUI_BLOCK_KEY, role?.permissions?.blockRainGui === true ? true : undefined);
  } catch (e) {}
...
function setUtilityRoleTags(player, enabledTags) {
  for (const entry of UTILITY_ROLE_TAGS) {
    ...
    if (shouldHave && !has) player.addTag(entry.tag);
...
async function openRolePresetsMenu(caller, target) {
  ...
  const presets = [
    ...
    {
      label: '§dCreative Builder',
      apply: () => {
        setPrimaryRankTag(target, 'rank:member');
        setUtilityRoleTags(target, new Set(['creative_builder']));
      },
    },
    {
...
async function openAssignRolesMenu(caller, target = null) {
  if (!hasPermission(caller, 'manageRoles')) {
    notify(caller, 'admin_error', '§c§l[ERROR]§r', '§cNo permission to manage roles.', 'note.bass');
    return;
  }
...
function syncFrozenPlayer(player, notifyOnRestore = false) {
  ...
  if (!isFrozenPersisted(player)) {
    frozenPlayerIds.delete(safePlayerId(player));
    return false;
  }
...
if (world.afterEvents?.playerSpawn) {
  world.afterEvents.playerSpawn.subscribe((ev) => {
    ...
    system.runTimeout(() => {
      if (!isLivePlayer(player)) return;
      const ban = findBanEntry(player);
      if (!ban) return;

      const reason = ban.reason ? `Banned: ${ban.reason}` : 'Banned from this server.';
      kickPlayer(null, player, reason).catch(() => {});
      notify(player, 'admin_ban_join', '§c§l[BANNED]§r', `§c${ban.reason || 'You are banned from this server.'}`, 'note.bass');
    }, 5);
  });
}
...
async function editRolePermissionsMenu(player, role, index) {
  ...
  if (nextPerms.blockRainGui === true || perms.blockRainGui === true) {
    for (const target of world.getAllPlayers()) {
      if (!target.hasTag(role.tag || `rank:${role.id}`)) continue;
      if (syncRainGuiBlockState(target)) {
        stripRainGuiForGravestone(target);
      }
    }
  }
...
function hasBuilderHubAccess(player) {
  return hasTagIgnoreCase(player, 'creative_builder') || hasPermission(player, 'creativeBuilderHub');
}

function hasTesterHubAccess(player) {
  return isTester(player) || hasPermission(player, 'testerHub');
}

function canOpenAdminPanel(player) {
  return isStaffRank(player) || hasPermission(player, 'openAdminPanel');
}
...
function getHubGamemodeLabel(player) {
  ...
  return labels[resolveGamemodeKey(player)] || '§8Unknown';
}
...
async function openAnnouncementForm(caller) {
  if (!canUseAdminControls(caller)) {
    notify(caller, "admin_error", "§c§l[ERROR]§r", "§cNo permission.", "note.bass");
    return;
  }

  const modal = new ModalFormData()
    .title('bd.modal:Announcement')
    .textField('Message (supports § colors)', 'Server announcement...');

  const res = await modal.show(caller);
  if (res.canceled) return;
  ...
  for (const player of world.getAllPlayers()) {
    notify(player, 'server_announce', '§6§l[ANNOUNCEMENT]§r', message, 'random.orb');
  }

  notify(caller, "admin_announce_sent", "§a§l[SENT]§r", "§aAnnouncement broadcast to all online players.", "random.orb");
}
...
async function openAdminPanel(caller) {
  while (true) {
        ...
        const main = new ActionFormData()
          ...
          .button(canUseAdminControls(caller) ? '§eAnnouncement' : '§7Announcement (no perm)');

        if (showBuilderHub) main.button('§dCreative Builder Hub');
        if (showTesterHub) main.button('§6Tester Hub');

        const announceIdx = 4;
        const builderIdx = showBuilderHub ? 5 : -1;
        const testerIdx = showTesterHub ? (showBuilderHub ? 6 : 5) : -1;
        ...
        if (res.selection === announceIdx) {
          await openAnnouncementForm(caller);
          continue;
        }

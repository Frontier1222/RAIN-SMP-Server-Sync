...
/** Server cap for infestation potion effect length (all bottle types). */
export const INFESTED_POTION_DURATION_TICKS = 30 * 20;
...
function blockInfestedPotionUse(player) {
    notify(
        ...
        '§c[RESTRICTED]',
        '§7Infestation potions require the Infested role.',
        'note.bass'
    );
}
...
function stripInfestedPotionsFromPlayer(player, notifyPlayer = false) {
    if (!player || hasInfestedRole(player)) return false;
...
/** Staggered realm tick — one cap + one sync candidate per master loop pass. */
...
function applyInfestedEffectNearby(source, radius) {
    ...
    if (source.typeId === 'minecraft:player' && !hasInfestedRole(source)) return;

    const location = source.location;
    for (const entity of source.dimension.getEntities({ location, maxDistance: radius })) {
        if (!entity?.isValid?.()) continue;
        if (entity.typeId === 'minecraft:player' && !hasInfestedRole(entity)) continue;
...
export function capInfestedEffectOnEntity(entity) {
    ...
    if (entity.typeId === 'minecraft:player' && !hasInfestedRole(entity)) {
    ...
    let effect;
    try {
        effect = entity.getEffect('minecraft:infested');
    } catch (e) {
        return;
    }

    if (!effect || effect.duration <= INFESTED_POTION_DURATION_TICKS) return;

    const amplifier = effect.amplifier ?? 0;
    const showParticles = effect.showParticles ?? true;
...
export function startInfestedPotionRuntime() {
    if (world.beforeEvents?.effectAdd) {
        world.beforeEvents.effectAdd.subscribe((event) => {
            ...
            if (event.entity?.typeId === 'minecraft:player' && !hasInfestedRole(event.entity)) {
                event.cancel = true;
                system.run(() => removeInfestedEffectFromEntity(event.entity));
                return;
            }

            const duration = getEffectAddDuration(event);
            if (duration > INFESTED_POTION_DURATION_TICKS) {
                event.duration = INFESTED_POTION_DURATION_TICKS;
            }
        });
    }
    ...
    if (world.afterEvents?.effectAdd) {
        world.afterEvents.effectAdd.subscribe((event) => {
            ...
            if (getEffectAddDuration(event) <= INFESTED_POTION_DURATION_TICKS) return;

            system.run(() => capInfestedEffectOnEntity(event.entity));
        });
    }
    ...
    if (world.afterEvents?.itemCompleteUse) {
        world.afterEvents.itemCompleteUse.subscribe((event) => {
            ...
            system.run(() => capInfestedEffectOnEntity(player));
        });
    }
    ...
    if (world.beforeEvents?.itemUse) {
        world.beforeEvents.itemUse.subscribe((event) => {
            ...
            if (isInfestedPotionStack(item) && !hasInfestedRole(player)) {
                event.cancel = true;
                system.run(() => {
                    stripInfestedPotionsFromPlayer(player, true);
                });
            ...
            if (item.typeId === INFESTED_SPLASH_POTION_30_ID) {
                event.cancel = true;
                system.run(() => {
                    try {
                        consumeHeldStack(player, item);
                        applyInfestedEffectNearby(player, 4);
                    } catch (e) {}
                });
                return;
            }

            if (item.typeId === INFESTED_LINGERING_POTION_30_ID) {
                event.cancel = true;
                system.run(() => {
                    try {
                        consumeHeldStack(player, item);
                        applyInfestedEffectNearby(player, 3);
                    } catch (e) {}
                });
                return;
            }

            if (!isInfestedPotionStack(item)) return;
            if (getPotionKind(item.typeId) !== 'potion') return;

            event.cancel = true;

            system.run(() => {
                try {
                    player.addEffect('infested', INFESTED_POTION_DURATION_TICKS, {
                        amplifier: 0,
                        showParticles: true,
                    });
                    consumeHeldStack(player, item);
    ...
    if (world.afterEvents?.playerSpawn) {
        world.afterEvents.playerSpawn.subscribe((event) => {
            ...
            system.run(() => {
                if (!hasInfestedRole(event.player)) {
                    stripInfestedPotionsFromPlayer(event.player);
                    removeInfestedEffectFromEntity(event.player);
                    return;
                }

                clearPlayerInfestedPotionsReady(event.player);
                syncPlayerInfestedPotions(event.player);
            });
        });
    }
}
...
import {
    ...
    clearArmorStandRestoreState,
    applyArmorStandEquipmentRestore,
    vacuumArmorStandTheftDrops,
} from '../../utils/claimDecor.js';
import { findPlayerPermRecord, isClaimPermAllowed, hasRoleClaimBypass, isClaimProtectionEnabled } from '../../utils/claimPermissions.js';
...
function blockDecorEntityUse(player, target, ev) {
    ...
    if (isEntityItemFrame(target.typeId)) {
        ...
        const snap = snapshotDecorEntity(target) || peekPendingArmorStandRestore(target.id);
        if (snap) queueArmorStandRestore(target);
        registerArmorStandBreakWatch(player, target, plot);
        system.run(() => {
            vacuumArmorStandTheftDrops(snap, player);
            if (snap) applyArmorStandEquipmentRestore(snap, player);
        });
    }
    ev.cancel = true;
    denyDecor(player, plot);
...
export const plotEntityKillProtection = {
    ...
    run: async (ev) => {
        ...
        if (isProtectedDecorEntity(hurtEntity.typeId)) {
            if (!attacker || shouldProtectDecor(plot, attacker)) {
                ...
                if (attacker && isArmorStandEntity(hurtEntity.typeId) && shouldProtectDecor(plot, attacker)) {
                    const snap = snapshotDecorEntity(hurtEntity) || peekPendingArmorStandRestore(hurtEntity.id);
                    if (snap) queueArmorStandRestore(hurtEntity);
                    registerArmorStandBreakWatch(attacker, hurtEntity, plot);
                    ev.cancel = true;
                    denyDecor(attacker, plot);
                    system.run(() => {
                        vacuumArmorStandTheftDrops(snap, attacker);
                        if (snap) applyArmorStandEquipmentRestore(snap, attacker);
                    });
                    return;
                }
                ev.cancel = true;
        ...
        if (!attacker) {
            if (isClaimProtectionEnabled(plot, 'protectEntityKill')) {
                ev.cancel = true;
            }
...
export const plotDecorEntityDieProtection = {
    ...
    run: async (ev) => {
        ...
        if (!shouldProtectDecor(plot, attacker)) {
            clearArmorStandRestoreState(deadEntity.id);
            return;
        }
...
export const plotDecorEntityDieRestoreProtection = {
    ...
    run: async (ev) => {
        ...
        if (isProtectedDecorEntity(deadEntity.typeId)) {
            const plot = getClaimAtLocation(deadEntity.location, deadEntity.dimension.id);
            const attacker = getDecorDamagePlayer(ev.damageSource);
        ...
        if (snap) {
            const attacker = getDecorDamagePlayer(ev.damageSource);
            vacuumArmorStandTheftDrops(snap, attacker);
            if (isArmorStandEntity(snap.typeId)) {
                applyArmorStandEquipmentRestore(snap, attacker);
            } else {
                restoreDecorEntity(snap, attacker);
            }
        }
    }
};

...
import { isRainGuiBlocked, hasPermission } from '../systems/ranks.js';
import { syncPlayerRainGuiGlint } from "../utils/rainGui.js";
import { getAllBounties, placeBounty, clearBounty } from '../systems/bounty.js';

const DEFAULT_DAILY_REWARD_ITEMS = ['viberater:rain_common_coin'];
...
function openBountyMenu(player) {
    ...
    const lines = Object.entries(bounties).map(
        ([name, data]) => `§e${name}§7 — §6${data.amount}§7 coins (by §f${data.placedBy}§7)`
    );
    const body = [
        lines.length ? lines.join('\n') : '§7No active bounties.',
        '',
        '§7Bounty targets can be attacked in safe zones and PVP-disabled claims.',
        '§7They cannot open chests in claims they do not own.',
    ].join('\n');

    const form = new ActionFormData()
        .title('bd.action:§6§lBounty')
        .body(body)
        .button('§a§lPlace Bounty §r§8· §7Reward a player')
        .button('§c§lRemove Bounty §r§8· §7Staff only')
        .button('§7§l« Back');
        ...
        form.show(player).then((res) => {
            ...
            if (res.selection === 0) {
                ...
                if (!players.length) {
                    ...
                    return openBountyMenu(player);
                }

                const pick = new ActionFormData()
                    .title('bd.action:§6Select Target')
                    .body('§7Choose a player to place a bounty on.');
                players.forEach((p) => pick.button(`§e§l${p.name}`));
                pick.button('§7§l« Back');

                system.runTimeout(() => {
                    pick.show(player).then((pickRes) => {
                        if (pickRes.canceled || pickRes.selection === players.length) return openBountyMenu(player);

                        const target = players[pickRes.selection];
                        const modal = new ModalFormData()
                            .title('bd.modal:Place Bounty')
                            .textField('Reward amount (Rain coins)', '100', '100');

                        system.runTimeout(() => {
                            modal.show(player).then((modalRes) => {
                                if (modalRes.canceled) return openBountyMenu(player);
                                const amount = Math.floor(Number(modalRes.formValues?.[0]) || 0);
                                if (amount <= 0) {
                                    sendError(player, 'Invalid bounty amount.');
                                    return openBountyMenu(player);
                                }
                                if (placeBounty(target.name, amount, player.name)) {
                                    sendSuccess(player, `§aBounty placed on §e${target.name}§a for §6${amount}§a coins.`);
                                    notify(target, 'bounty_placed', '§6§l[BOUNTY]§r', `§e${player.name}§7 placed a §6${amount}§7 coin bounty on you!`, 'random.levelup');
                                }
            ...
            if (res.selection === 1) {
                ...
                if (!names.length) {
                    sendError(player, 'No active bounties to remove.');
                    return openBountyMenu(player);
                }

                const canClear = hasPermission(player, 'openAdminPanel');
                if (!canClear) {
                    sendError(player, 'Only staff can remove bounties.');
                    return openBountyMenu(player);
                }

                const pick = new ActionFormData()
                    .title('bd.action:§cRemove Bounty')
                    .body('§7Select a bounty to clear.');
                names.forEach((name) => pick.button(`§c§l${name} §r§8· §7Clear bounty`));
                pick.button('§7§l« Back');

                system.runTimeout(() => {
                    pick.show(player).then((pickRes) => {
                        if (pickRes.canceled || pickRes.selection === names.length) return openBountyMenu(player);
                        const name = names[pickRes.selection];
                        clearBounty(name);
                        sendSuccess(player, `§aRemoved bounty on §e${name}§a.`);
                        openBountyMenu(player);
                    });
                }, 2);
            }
                            ...
                            permMenu.show(player).then(pres => {
                                ...
                                if (pres.selection === 0) {
                                    ...
                                    const modal = new ModalFormData()
                                        ...
                                        .toggle("Disable Raids", {
                                            defaultValue: defPerms.protectRaid ?? true
                                        })
                                    ...
                                    system.runTimeout(() => {
                                        modal.show(player).then(mres2 => {
                                            ...
                                            selectedPlot.permissions.default.protectRaid = mres2.formValues[12];
                                            selectedPlot.permissions.default.allowHomes = !!mres2.formValues[13];

...
export const ROLE_PERMISSION_DEFS = [
  ...
  { key: "hideSidebar", label: "Hide Stats Sidebar (/bd:hidesidebar)" },
];
...
/** Role permission — blocks opening / receiving the Rain GUI (not overridden by staff auto-perms). */
export const RAIN_GUI_BLOCK_KEY = "nf.rain_gui_blocked";

function playerHasRainGuiBlockRole(player) {
  for (const cr of getCustomRoles()) {
    if (cr.permissions?.blockRainGui !== true) continue;
    const tag = cr.tag || `rank:${cr.id}`;
...
export function isRainGuiBlocked(player) {
  ...
  try {
    if (player.getDynamicProperty(RAIN_GUI_BLOCK_KEY) === true) return true;
  } catch (e) {}

  return playerHasRainGuiBlockRole(player);
}

/** Call after role tag / permission changes. */
export function syncRainGuiBlockState(player) {
  if (!player) return false;
  const blocked = playerHasRainGuiBlockRole(player);
  try {
    player.setDynamicProperty(RAIN_GUI_BLOCK_KEY, blocked ? true : undefined);
  } catch (e) {}
  return blocked;
}

/** Toggle HUD sidebar via /bd:hidesidebar — staff bypass; others need hideSidebar on their role. */
export function canHideSidebar(player) {
  if (!player) return false;
  if (
    player.hasTag("staff") ||
    player.hasTag("rank:admin") ||
  ...
  return hasPermission(player, "hideSidebar");
}
